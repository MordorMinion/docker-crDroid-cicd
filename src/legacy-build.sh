...
if [ "$LOCAL_MIRROR" = true ]; then
  ...
  if [ "$INIT_MIRROR" = true ]; then
    if [ ! -d .repo ]; then
      ...
      ( yes||: ) | repo init -u https://github.com/crDroid/android --mirror --no-clone-bundle -p linux --git-lfs &>> "$repo_log"
    fi
  else
  ...
  if [ -n "$branch" ] && [ -n "$devices" ]; then
    ...
    case "$branch" in
      ...
      16.0*)
        ...
        android_version="16"
        frameworks_base_patch="android_frameworks_base-Android16.patch"
        modules_permission_patch="packages_modules_Permission-Android16.patch"
        user_build_spoofing_patch="android_frameworks_base-user_build.patch"
        ;;
      *)
    ...
    if [ "$CALL_REPO_INIT" = true ]; then
      ...
      if [ "$LOCAL_MIRROR" = true ]; then
        ( yes||: ) | repo init -u https://github.com/crdroidandroid/android.git --reference "$MIRROR_DIR" -b "$branch" --git-lfs &>> "$repo_log"
      else
        ( yes||: ) | repo init -u https://github.com/crdroidandroid/android.git -b "$branch" --git-lfs &>> "$repo_log"
      fi
    else
